      ...
      {payload.map((p: TooltipPayloadItem, i: number) => (
        ...
        >
          ...
          <span className="font-semibold">
            ...
            p.name?.toLowerCase().includes("transaksi")
              ? fmtNum(p.value)
              : fmt(Number(p.value))}
          </span>
        </div>
...
function AccountBankBalanceDashboard({
  ...
  const topAccount = accountDetails.reduce<AccountDetail | null>(
    (acc, cur) => (!acc || cur.totalRevenue > acc.totalRevenue ? cur : acc),
    null,
  );

  const avgPerAccount =
    summary.totalAccountBanks > 0
      ? Math.round(summary.totalRevenue / summary.totalAccountBanks)
      : 0;
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <KPICard
          ...
          sub={
            isLoading
              ? undefined
              : `${fmtNum(summary.totalTransaction)} transaksi`
          }
        ...
        <KPICard
          ...
          value={isLoading ? "—" : fmtNum(summary.totalAccountBanks)}
          sub="Rekening bank terdaftar"
          icon={Landmark}
          color={CHART_SERIES.info}
          loading={isLoading}
        />
        <KPICard
          title="Rata-rata per Rekening"
          value={isLoading ? "—" : fmt(avgPerAccount)}
          sub="Saldo masuk rata-rata per rekening"
          icon={Wallet}
        ...
        <TabsContent value="overview" className="mt-4 space-y-4">
            ...
            <Card className="lg:col-span-2">
              <CardHeader className="pb-2">
                <CardTitle className="text-base">
                  Saldo Masuk per Bulan
                </CardTitle>
                <CardDescription className="text-xs">
                  Total saldo yang masuk ke seluruh rekening per bulan
                </CardDescription>
              </CardHeader>
              <CardContent>
                {isLoading ? (
          ...
          <Card>
            <CardHeader className="pb-2">
              ...
              <CardDescription className="text-xs">
                Diurutkan berdasarkan total saldo masuk pada periode ini
              </CardDescription>
            </CardHeader>
        ...
        <TabsContent value="bank" className="mt-4 space-y-4">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            <Card>
              <CardHeader className="pb-2">
                <CardTitle className="text-base">
                  Distribusi Saldo per Bank
                </CardTitle>
                <CardDescription className="text-xs">
                  Porsi total saldo masuk berdasarkan nama bank
                </CardDescription>
              </CardHeader>
              <CardContent>
                {isLoading ? (
                    ...
                    <ResponsiveContainer width="100%" height={220}>
                      <PieChart>
                        ...
                        <Tooltip
                          formatter={(value) =>
                            value
                              ? [fmtFull(Number(value)), "Saldo Masuk"]
                              : ["", ""]
                          }
            ...
            <Card>
              <CardHeader className="pb-2">
                <CardTitle className="text-base">
                  Saldo Masuk per Bank
                </CardTitle>
                <CardDescription className="text-xs">
                  Nominal saldo masing-masing bank
                </CardDescription>
              </CardHeader>
              <CardContent>
                {isLoading ? (
          ...
          {!isLoading && byBankGroup.length > 0 && (
            <Card>
              <CardHeader className="pb-2">
                ...
                <CardDescription className="text-xs">
                  Termasuk collection rate masing-masing kelompok bank
                </CardDescription>
              </CardHeader>
              <CardContent className="p-0">
                <div className="overflow-x-auto">
