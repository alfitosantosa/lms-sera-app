...
function UploadBilling({
    ...
    try {
      ...
      if (sheetNames.length > 0) {
        ...
        if (sheetNames.length > 1) {
          toast.success(
            `Ditemukan ${sheetNames.length} sheet. Pilih sheet di dropdown untuk melihat data.`,
          );
        } else {
    ...
    try {
      ...
      for (let i = 1; i < rows.length; i++) {
        ...
        if (!studentId) errors.push("studentId kosong");
          ...
          errors.push("studentId tidak ditemukan");

        if (!paymentTypeId) errors.push("paymentTypeId kosong");
        else if (!paymentTypeMap.has(paymentTypeId))
          errors.push("paymentTypeId tidak ditemukan");

        if (quantity <= 0) errors.push("quantity harus > 0");
        if (amount < 0) errors.push("amount tidak boleh negatif");
        if (!month || !/^\d{2}$/.test(month))
          errors.push("month format salah (01-12)");
        if (!year || !/^\d{4}$/.test(year))
          errors.push("year format salah (YYYY)");
        ...
        parsed.push({
          ...
          _paymentTypeName:
            paymentTypeMap.get(paymentTypeId)?.name ?? paymentTypeId,
          _errors: errors,
        });
      }
      ...
      if (errorCount > 0) {
        toast.warning(
          `Sheet "${targetSheet}": ${parsed.length} baris ditemukan, ${errorCount} baris memiliki error`,
        );
      } else {
        toast.success(
          `Sheet "${targetSheet}": ${parsed.length} baris siap diupload`,
        );
      }
    } catch (err) {
      console.error(err);
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      setUploadResult(
        result as { count: number; skipped: number; total: number },
      );
      toast.success(
        `Berhasil membuat ${(result as { count: number }).count ?? validRows.length} item tagihan!`,
      );
      setFiles([]);
      setPreviewRows([]);
      ...
      const errorMessage =
        err instanceof Error ? err.message : "Gagal mengupload data";
  ...
  const exportPaymentTypeList = async () => {
    try {
      ...
      const wsData = [
        [
          "ID (gunakan di kolom Payment Type ID)",
          "Nama",
          "Nominal",
          "Owner",
          "Tipe SKU",
          ...
          <div className="flex items-start gap-2">
            ...
            <div className="text-sm text-info-strong space-y-1">
              ...
              <ul className="list-disc list-inside space-y-0.5 text-info-strong">
                ...
                <li>
                  <strong>Payment Type ID</strong> — ambil dari export Jenis
                  Tagihan di Langkah 1
                </li>
                <li>
                  <strong>Quantity</strong> — jumlah item (angka bulat, minimal
                  1)
                </li>
                <li>
                  <strong>Amount</strong> — nominal per satuan (angka, tanpa
                  titik/koma)
                </li>
                <li>
                  <strong>Subtotal</strong> — boleh dikosongkan, otomatis = Qty
                  × Amount
                </li>
                <li>
                ...
                <li>
                  <strong>Name</strong> — nama item tagihan (mis. "SPP Januari{" "}
                  {currentYear}")
                </li>
                <li>
                  <strong>SKU Type</strong> — lihat tabel SKU Type di bawah
                  (opsional)
                </li>
                <li>
                  ...
                  <TableRow key={pt.id}>
                    ...
                    <TableCell>
                      <CopyButton
                        ...
                        content={pt.id}
                        onClick={() =>
                          toast.success(`ID ${pt.name} berhasil dicopy`)
                        }
                      />
                    </TableCell>
                  </TableRow>
              ...
              {skuTypes.map((sku) => (
                <div key={sku} className="flex items-center gap-1.5">
                  ...
                  <CopyButton
                    ...
                    content={sku}
                    onClick={() =>
                      toast.success(`SKU "${sku}" berhasil dicopy`)
                    }
                  />
                </div>
              ))}
                    ...
                    >
                      ...
                      <div className="flex items-center gap-1">
                        ...
                        <CopyButton
                          ...
                          onClick={() =>
                            toast.success(
                              `Bulan ${name} (${num}) berhasil dicopy`,
                            )
                          }
                        />
                      </div>
                  ...
                  >
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      <CopyButton
                        ...
                        content={y}
                        onClick={() =>
                          toast.success(`Tahun ${y} berhasil dicopy`)
                        }
                      />
                    </div>
                  </div>
