...
export function handlePrismaError(error: unknown): NextResponse {
  ...
  if (error instanceof Prisma.PrismaClientKnownRequestError) {
    switch (error.code) {
      ...
      case "P2003":
        return NextResponse.json(
          { success: false, message: "Data masih digunakan oleh data lain" },
          { status: 400 },
        );
      case "P2025":
        return NextResponse.json(
      ...
      default:
        ...
        return NextResponse.json(
          { success: false, message: `Kesalahan database: ${error.code}` },
          { status: 500 },
        );
    }
  }
  ...
  return NextResponse.json(
    { success: false, message: "Terjadi kesalahan tidak diketahui" },
    { status: 500 },
  );
}
