 ...
 */
export type Tenant = { ok: true; foundationId: string } | { ok: false; response: NextResponse };
 ...
 */
export async function resolveFoundation(request: NextRequest, explicit?: string | null): Promise<Tenant> {
  ...
  const [userData, user] = await Promise.all([
    prisma.userData.findUnique({ where: { userId }, select: { foundationId: true } }),
    prisma.user.findUnique({ where: { id: userId }, select: { foundationId: true } }),
  ]);

  const sessionFoundationId = userData?.foundationId ?? user?.foundationId ?? null;

  if (!sessionFoundationId) return { ok: false, response: tenantForbidden() };

  if (explicit && explicit !== sessionFoundationId) {
    return { ok: false, response: tenantForbidden("Tidak punya akses ke data yayasan ini") };
  }
