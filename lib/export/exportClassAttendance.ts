...
export const exportClassAttendanceDailyToExcel = async (
  ...
  try {
    ...
    const dailyData = Object.entries(attendanceByDate).map(
      ([date, attendances]) => {
    ...
    const studentSummaryData = students.map((student, index) => {
      const studentAttendances = allAttendanceRecords.filter(
      ...
      const percentage =
        totalRecords > 0 ? Math.round((totalPresence / totalRecords) * 100) : 0;
    ...
    const exportFilename =
      filename ||
      `rekap-absensi-kelas-${classData.name.replace(/\s+/g, "-")}-${format(new Date(), "yyyy-MM-dd")}.xlsx`;
